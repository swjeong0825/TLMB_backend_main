...
# ---------------------------------------------------------------------------
# v8 round-trip and v1/v2/v3/v4/v5/v6 upgrade
# ---------------------------------------------------------------------------


def test_from_dict_v8_round_trip() -> None:
    raw = {
        "version": 8,
...
def test_from_dict_v5_round_trip_inverts_require_allowlist_to_v8() -> None:
    """v5 `require_allowlist=True` becomes v8 `auto_register_players_on_match=False`
...
def test_from_dict_v4_legacy_key_round_trip_upgrades_to_v8() -> None:
    """v4 `require_eligible_players=True` becomes v8 `auto_register_players_on_match=False`."""
    raw_v4 = {
        "version": 4,
        "pair_matchup_idempotency": "none",
        "one_pair_per_player": True,
...
def test_from_dict_v3_round_trip_upgrades_to_v8_with_default_flag() -> None:
    """v3 inputs are accepted and upgraded transparently to v8 with
...
def test_from_dict_v1_input_is_upgraded_to_v8_with_defaults() -> None:
    raw_v1 = {
        "version": 1,
        "pair_matchup_idempotency": "once_per_league",
        "one_pair_per_player": True,
...
def test_from_dict_v1_input_with_otpp_false_upgrades_to_v8() -> None:
...
def test_from_dict_v2_input_upgrades_to_v8() -> None:
    raw_v2 = {
        "version": 2,
        "pair_matchup_idempotency": "once_per_league",
        "one_pair_per_player": True,
...
def test_default_for_new_league_is_v8_with_auto_register_on_and_once_per_day() -> None:
...
@pytest.mark.parametrize(
    ...
    [
        {},
        {"version": 1},
        {"version": 1, "pair_matchup_idempotency": "none", "one_pair_per_player": "yes"},
        {"version": 9, "pair_matchup_idempotency": "none", "one_pair_per_player": True},
    ],
)
...
# ---------------------------------------------------------------------------
# v3 cross-rule and OTPP=false acceptance (preserved verbatim under v8)
# ---------------------------------------------------------------------------


def test_from_dict_accepts_pair_subject_with_otpp_false() -> None:
    rules = LeagueRules.from_dict(
        {
            "version": 6,
            "pair_matchup_idempotency": "once_per_league",
...
def test_from_dict_rejects_player_subject_with_otpp_true() -> None:
    """v3 cross-rule (still enforced under v8): `ranking_subject='player'`
    requires `one_pair_per_player=false`."""
    ...
    assert "one_pair_per_player" in msg
...
def test_from_dict_v2_input_with_player_subject_and_otpp_true_is_rejected() -> None:
    ...
    even when the declared version is older than v8."""
    with pytest.raises(InvalidLeagueRulesError):
        LeagueRules.from_dict(
            {
                "version": 2,
