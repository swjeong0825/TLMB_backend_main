...
V3 is the next iteration of `LeagueRules` after [v2](17_configurable_ranking.md) (currently in production). V2 added configurable ranking (`ranking_subject` + ordered `tie_breakers`) but locked `one_pair_per_player` to `true`, which made `ranking_subject = "player"` mathematically equivalent to `ranking_subject = "pair"`. V3 unlocks `one_pair_per_player = false`, introduces the first `(ranking_subject, one_pair_per_player)` cross-rule, and ships the read-path revisions that make player-subject leagues genuinely distinct from pair-subject leagues.
...
- Loosen `LeagueRules.from_dict` so `one_pair_per_player = false` is accepted.
- Introduce the first cross-rule: `ranking_subject = "player"` requires `one_pair_per_player = false`. Equivalently, `one_pair_per_player = true` forces `ranking_subject = "pair"`.
- Bump `LeagueRules.version` to `3`. v1 and v2 inputs continue to be accepted on read and upgraded transparently.
- Update read paths that today assume "a player belongs to at most one pair":
  - `GetStandingsByPlayerUseCase` returns **all** of the player's pair rows under `(pair, OTPP=false)` (one entry per pair).
  - `GetMatchHistoryByPlayerUseCase` returns matches across **all** of the player's pairs (union, deduped by `match_id`).
- Ship an alembic migration (`004`) that auto-rewrites every existing `(player, OTPP=true)` row to `(pair, OTPP=true)`, preserving `tie_breakers` verbatim.
...
- A submit-time check for a player on pair1 *and* on a different pair that is pair2 — already enforced by the existing `SamePlayerOnBothPairsError` (compares nicknames, runs before any aggregate is loaded). v3 trusts this, the calculator's same-player-on-both-sides guard becomes a defense-in-depth check only.
...
|---|---|---|---|
| `(pair, OTPP=true)` | legal | legal | Default for new leagues; unchanged byte-for-byte. |
| `(player, OTPP=true)` | legal but mathematically equivalent to `(pair, OTPP=true)` | **illegal** (cross-rule rejection) | Existing prod rows are auto-rewritten to `(pair, OTPP=true)` by alembic 004. |
| `(pair, OTPP=false)` | illegal (OTPP=false rejected) | **legal** | A player may belong to multiple pairs; pair rows aggregate match outcomes per pair. |
| `(player, OTPP=false)` | illegal (OTPP=false rejected) | **legal** | Player rows aggregate match outcomes across every pair the player belongs to. This is the case where v2's player-subject path becomes genuinely useful. |

### Why introduce the cross-rule now

`(player, OTPP=true)` produces standings that are *mathematically equivalent* to `(pair, OTPP=true)` (every pairmate has identical metric tuples; see [17_configurable_ranking.md §"Worked example"](17_configurable_ranking.md#worked-example-equivalence-under-otpptrue)). Allowing it in v3 once `OTPP=false` exists would offer two ways to express "rank by pair" with no observable difference, increasing schema ambiguity for no UX win. Rejecting it converges the schema on a state where every legal combo conveys distinct information.

### Why `(pair, OTPP=false)` is legal and meaningful

Under OTPP=false, a player can be on multiple pairs (e.g. partnered with Bob in one pair, and with Charlie in another). Pair-subject standings still rank one row per pair. Each pair's metrics aggregate only that pair's matches. This is what most "casual round-robin where partners rotate" leagues actually want.

### Migration impact for existing leagues

V2 leagues all have `one_pair_per_player = true` (locked at validation time), so the only existing combo that becomes illegal under v3 is `(player, OTPP=true)`. Migration 004 rewrites every such row to `(pair, OTPP=true)`:

- `ranking_subject` flips from `"player"` to `"pair"`.
- `tie_breakers` is preserved verbatim — host metric choices are not lost.
- `one_pair_per_player` and `pair_matchup_idempotency` are untouched.
...
Affected leagues' standings will visibly change shape — rows collapse from one-per-player back to one-per-pair, halving the row count. The v3 release notes must call this out so league hosts are not surprised.
...
| `pair_matchup_idempotency` | str | `"none"`, `"once_per_league"` | `"once_per_league"` (unchanged) |
| `one_pair_per_player` | bool | `true` or `false` | `true` (unchanged from v1/v2 product default) |
| `ranking_subject` | str | `"pair"`, `"player"` | `"pair"` (unchanged from v2) |
...
The product default for new leagues stays `(pair, OTPP=true, ["matches_won"])` — byte-identical to v2 defaults. v3 only widens the legal input set; it does not change what the create-league form sends when the user accepts defaults.
...
1. `version` must be `1`, `2`, or `3`.
   - v1 inputs inject `ranking_subject="pair"`, `tie_breakers=["matches_won"]`, then upgrade to v3.
...
2. `pair_matchup_idempotency` must be `"none"` or `"once_per_league"` (unchanged).
3. `one_pair_per_player` must be a boolean (no value-restriction; both `true` and `false` are legal).
4. `ranking_subject` must be `"pair"` or `"player"`.
5. `tie_breakers` must be a non-empty list of allowed metrics with no duplicates.
6. **Cross-rule (new in v3):** if `ranking_subject == "player"` and `one_pair_per_player is True`, raise `InvalidLeagueRulesError`.

The cross-rule message text should be explicit and actionable: e.g. `"ranking_subject='player' requires one_pair_per_player=false; pick (pair, OTPP=true) or (player, OTPP=false)"`.
...
- For `ranking_subject == "pair"`: each pair row aggregates only the matches that pair played. A player who is on multiple pairs contributes their per-match score to whichever pair they were partnered with for that specific match.
- For `ranking_subject == "player"`: each player row aggregates the player's per-match scores across **every pair** they belong to. Partners may differ across matches; the player accumulates wins/losses/games regardless.

---

## Read-path contracts under OTPP=false

The endpoints `GET /leagues/{id}/standings/by-player` and `GET /leagues/{id}/matches/by-player` resolve a player by nickname. Under OTPP=true these were trivially scoped to "the player's one pair". Under OTPP=false the contract widens.

### `GET /leagues/{id}/standings/by-player`

| `ranking_subject` | OTPP | Behavior |
|---|---|---|
| `"pair"` | `true` | Returns the single row of the player's one pair (unchanged from v2). |
| `"pair"` | `false` | Returns **all** rows for every pair the player belongs to. The `standings` array may have multiple entries. |
...
The empty-result case (player exists but has no pair — e.g. all their pairs were deleted) still returns an empty `standings` array.

### `GET /leagues/{id}/matches/by-player`

| OTPP | Behavior |
|---|---|
| `true` | Returns matches for the player's one pair (unchanged from v2). |
| `false` | Returns the **union** of matches across every pair the player belongs to, deduped by `match_id`, sorted by `created_at` descending. |

To support the OTPP=false case efficiently, `MatchRepository` gains a new method `get_all_by_player(league_id, player_id, pair_ids) -> list[Match]`. The implementation is a single SQL query: `WHERE league_id = :lid AND (pair1_id = ANY(:tids) OR pair2_id = ANY(:tids))`. Application-layer aggregation across multiple `get_all_by_pair` calls is also acceptable but less efficient.

---

## Calculation flow

Identical structure to v2 (see [17_configurable_ranking.md §"Calculation flow"](17_configurable_ranking.md#calculation-flow)). The `_compute_for_players` branch in `StandingsCalculator` is already implemented to aggregate per-match outcomes across every pair a player belongs to (`pairs_for_player[player_id]` is a `set`), so v2 shipped the OTPP=false-correct algorithm a release early. V3 simply validates that this branch now runs against genuinely-distinct partner sets in real data.

The same-player-on-both-sides guard inside the calculator (`if t1 in pair_ids and t2 in pair_ids: continue`) becomes a defense-in-depth check in v3. The primary defense is the existing submit-time `SamePlayerOnBothPairsError`, which compares nicknames *before* any aggregate is loaded — see `SubmitMatchResultUseCase`. Persisted match data should never trigger the calculator guard; if it does, that indicates a bug or a hand-edit of the `matches` table.
...
Note Alice and Bob tie on `matches_won` and **do not** tie on `games_diff` — under v2's OTPP=true equivalence example, pairmates always shared a rank. Under v3 OTPP=false they don't, because they didn't always partner together. This is the case the v2 player-subject branch was built to serve.

For comparison, the same matches under `ranking_subject = "pair"`, OTPP=false:
...
```sql
-- Rewrite (player, OTPP=true) → (pair, OTPP=true), preserving tie_breakers.
UPDATE leagues
...
The `(player, OTPP=true) → (pair, OTPP=true)` rewrite from upgrade is **not reversed** by downgrade. A row that was `(player, OTPP=true)` in v2 cannot be recovered after upgrade because the original `ranking_subject` value is overwritten in place. The migration's docstring documents this as a one-way data change. Operators who need a recovery path must take a manual JSONB snapshot of the `leagues.rules` column before running the upgrade.

This decision (best-effort downgrade rather than blocking-downgrade or snapshot-table) was made because:

- v2 standings for `(player, OTPP=true)` are mathematically equivalent to `(pair, OTPP=true)` standings, so the user-visible information is preserved by the rewrite — only the rendering shape changes.
...
These extend orthogonal axes (metrics, score shape, pair matchup rules) and can ship without bumping `LeagueRules.version` if they introduce only additional optional fields, or with a version bump if they meaningfully change validation.
...
| `backend_main/app/domain/aggregates/league/aggregate_root.py` | No code change. `register_players_and_pair` already gates `OnePairPerPlayerPolicy` on `self.rules.one_pair_per_player`. |
| `backend_main/app/domain/aggregates/league/policies.py` | No change. |
| `backend_main/app/domain/services/standings_calculator.py` | Replace the `# TODO(v3-ranking-tightening)` comment with a v3 explanation. Code unchanged — the `_compute_for_players` branch is already OTPP=false-correct. The same-player-on-both-sides guard (line 184) is documented as defense-in-depth; the primary defense is `SamePlayerOnBothPairsError` at submit time. |
| `backend_main/app/domain/aggregates/match/repository.py` | Add abstract method `get_all_by_player(league_id, player_id, pair_ids) -> list[Match]` (used by `GetMatchHistoryByPlayerUseCase` under OTPP=false). |

### Backend application layer

| File | Change |
|---|---|
| `backend_main/app/application/use_cases/get_standings_by_player_use_case.py` | Replace the `next(...)` pair lookup with a set-comprehension that collects every pair the player belongs to, then filters `all_entries` by `e.pair_id in player_pair_ids`. Remove the `# TODO(v3-ranking-tightening)` comment. |
| `backend_main/app/application/use_cases/get_match_history_by_player_use_case.py` | Replace the `next(...)` pair lookup + single `get_all_by_pair` call with a single `get_all_by_player(league_id, player_id, pair_ids)` call. Dedupe by `match_id` (the new repo method does this server-side). |
| `backend_main/app/application/use_cases/submit_match_result_use_case.py` | No change. The existing `SamePlayerOnBothPairsError` check already prevents the only pathological OTPP=false case the calculator's defense-in-depth guard would catch. |
...
| File | Change |
|---|---|
| `backend_main/alembic/versions/004_leagues_rules_v3.py` | **NEW.** Implements the two `UPDATE` statements above. `revision = "004"`, `down_revision = "003"`. Docstring documents the irreversible `(player, OTPP=true) → (pair, OTPP=true)` rewrite. |
| `backend_main/app/infrastructure/persistence/repositories/match_repository.py` | Implement `get_all_by_player`. SQL: `SELECT … FROM matches WHERE league_id = :lid AND (pair1_id = ANY(:tids) OR pair2_id = ANY(:tids))`. |
...
| File | Change |
|---|---|
| `frontend/frontend_vanilla/create-league/index.html` | Insert a new `<select name="one_pair_per_player">` block in `details.create-league-advanced`, positioned between the `pair_matchup_idempotency` and `ranking_subject` controls. |
| `frontend/frontend_vanilla/js/create-league.js` | In `buildPayload`, send `version: 3` and the actual OTPP boolean. Add a `change`-event handler that enforces the cross-rule client-side: setting `ranking_subject = "player"` forces OTPP=false; setting OTPP=true forces `ranking_subject = "pair"`. Remove the `// TODO(v3-ranking-tightening)` comment. |
| `frontend/frontend_vanilla/js/i18n.js` | Add new keys (en + ko): `createLeague.labelOnePairPerPlayer`, `createLeague.optionOTPPTrue`, `createLeague.optionOTPPFalse`, `createLeague.crossRuleHint`. The legacy `createLeague.onePairPerPlayer` key may be deprecated. |
...
| `chat_to_intent_server_fastapi/design_doc/02_read_only_backend_endpoints.md` | Add a note under `GET /leagues/{league_id}/standings/by-player` that under v3 the endpoint may return multiple rows when the resolved player belongs to multiple pairs. |
...
- Flip `test_from_dict_rejects_otpp_false_with_pair_subject` → `test_from_dict_accepts_pair_subject_with_otpp_false`.
...
- Add: `register_players_and_pair` allows the same player on a second pair when `rules.one_pair_per_player is False`.
- Verify: same operation still raises `PairConflictError` when OTPP=true (regression).
...
- Add: `(pair, OTPP=false)` with a player on 2 pairs returns 2 rows.
...
- Add: under OTPP=false, a player on 2 pairs sees matches from both pairs in the result, deduped by `match_id`.

### Integration / migration

`backend_main/tests/integration/test_migration_004_leagues_rules_v3.py` (NEW):

- Seed three rows: `(pair, OTPP=true, version=2)`, `(player, OTPP=true, version=2)`, `(pair, OTPP=true, version=2, tie_breakers=["games_won","games_diff"])`.
- Run upgrade. Assert row 2 is rewritten to `pair`, all rows have `version=3`, row 3's `tie_breakers` preserved verbatim.
- Idempotency: re-run upgrade, assert no change.
- Downgrade: assert all rows back to `version=2`; assert row 2 still has `ranking_subject="pair"` (irreversible by design).
...
- [ ] E2E: create `(pair, OTPP=false)` league → submit Alice+Bob vs Charlie+Dan, then Alice+Eve vs Frank+Charlie → standings reflect per-pair aggregation.
...
- [ ] Existing prod leagues with `(player, OTPP=true)` are auto-rewritten to `(pair, OTPP=true)` on migration; `tie_breakers` preserved.
...
- [03_business_invariants.md](03_business_invariants.md) — invariants table; the conditional one-pair-per-player invariant becomes meaningfully conditional in v3.
