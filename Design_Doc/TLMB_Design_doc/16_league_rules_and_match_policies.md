...
- **Versioning:** A required integer field `version` (started at `1`; current is `6`). Parsers accept unknown keys for forward compatibility but validate known keys strictly. v1–v5 inputs are silently upgraded to v6 (v1 inputs additionally have the v2 ranking defaults injected before the later upgrades; v4 inputs map the legacy `require_eligible_players` key to v5 `require_allowlist`, and the v5→v6 step **inverts** that boolean and renames it to `auto_register_players_on_match`).
- **Fields (v6):**

| Field | Type (logical) | Meaning |
|-------|----------------|---------|
| `version` | int | Schema version; current is `6`. v1–v5 are accepted on input and upgraded transparently. |
