...
- Backed by: Computed on the fly from all Match records in the league + Pair and Player data from the League aggregate; no materialized table in V1
...
- Fields exposed:
  - rank (int — shared rank for tied pairs)
  - pair_id (str)
...
- Backed by: All Match records for the league, with pair_id references resolved to player nicknames from the League aggregate at read time
...
- Used by: Player (via leagueId) or League Host (via hostToken) viewing the current list of players and pairs in a league
- Backed by: Player and Pair entities from the League aggregate
...
- Fields exposed (pairs): pair_id (str), player1_nickname (str), player2_nickname (str)
- Why this is NOT a write aggregate: The roster is a direct projection of League aggregate state. Players and pairs are only created or mutated through the League aggregate root.
...
- Domain service used?: Yes — StandingsCalculator receives the loaded match list, pair list, and player list
...
- Notes: Match records are loaded via MatchRepository.get_all_by_league; pair-to-nickname resolution uses the League aggregate loaded via LeagueRepository.get_by_id
...
- Filters: scoped to the given league_id; further filtered to matches involving the resolved player's pair
- Sorting / pagination: sorted by created_at descending; no pagination in V1
- Output: list[MatchHistoryRecord] ordered by created_at descending (empty list if player has no current pair)
- Domain service used?: No
- Notes: Player is resolved by normalized nickname from the League aggregate. Pair is identified from league.pairs. Matches are loaded via MatchRepository.get_all_by_league and filtered in-memory. Nickname resolution uses current League state — same retroactive-edit behaviour as GetMatchHistory.
...
- Auth: leagueId possession (player-level access) or hostToken (admin access) — both accepted
- Filters: scoped to the given league_id
- Sorting / pagination: players sorted alphabetically by nickname; pairs sorted by player1_nickname alphabetically; no pagination in V1
- Output: RosterView (players: list[PlayerEntry], pairs: list[PairEntry])
- Domain service used?: No
