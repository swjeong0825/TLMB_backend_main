...
Leagues differ in *what they want to rank* (pairs or individual players) and *how they want to break ties* (matches won, game differential, win percentage, ...). Today, [`StandingsCalculator`](06_domain_services.md) hard-codes "rank pairs by wins descending, no tiebreaker" — this works for the simplest leagues but rules out common real-world rankings such as "wins, then game differential". This document adds a second dimension to [`LeagueRules`](16_league_rules_and_match_policies.md): a per-league **ranking subject** (pair vs. player) and an **ordered tie-breaker list**.

**Scope in this implementation (v2 of LeagueRules):**

- Add `ranking_subject: "pair" | "player"` and `tie_breakers: list[Metric]` fields to `LeagueRules`.
...
- Make the standings response polymorphic on `subject_kind` (a `"pair"` row or a `"player"` row).
- Reuse the existing `GET /leagues/{id}/standings` and `GET /leagues/{id}/standings/by-player` endpoints — no new routes.
- `one_pair_per_player` is **locked to `true`** in v2: `LeagueRules.from_dict` rejects any other value. Configurable OTPP arrives in v3, which is also when the `(ranking_subject, one_pair_per_player)` cross-rule first becomes meaningful — see [Forward-compatibility note](#forward-compatibility-note-planned-for-v3).
...
|---|---|---|---|
| `pair` | `pair_id` | both partner nicknames | matches won/lost by that pair |
| `player` | `player_id` | one nickname | matches won/lost by every pair this player belongs to |

Under v2, every league has `one_pair_per_player == true` (the field is locked at validation time), so "every pair this player belongs to" is effectively "the player's one pair". This makes player-subject rankings under v2 mathematically equivalent to pair-subject rankings, just rendered with one row per player. See [Worked example: equivalence under OTPP=true](#worked-example-equivalence-under-otpptrue) for why this is intentional.
...
For a subject row (a pair or a player), let `M` be the set of matches involving that subject:
...
| `games_won` | sum of the subject's per-match `int(pair_score)` | the integer already stored on `SetScore`; v2 makes no schema change |
| `games_lost` | sum of the *opponent's* per-match `int(pair_score)` | symmetric to `games_won` |
...
### `one_pair_per_player` locked to `true` (v2)

In v2, `LeagueRules.from_dict` rejects any rules object whose `one_pair_per_player` is not `true`. There is **no `(ranking_subject, one_pair_per_player)` cross-rule** in v2 — the constraint is unnecessary because OTPP is forced to `true` for every league, which trivially satisfies any pairing with `ranking_subject`. Allowing OTPP=false is meaningful (a player accumulating stats across multiple partners) but cannot ship safely until the read paths that assume one-pair-per-player — most notably `GetStandingsByPlayerUseCase`'s `next(...)` lookup — are revised. v3 unlocks `one_pair_per_player = false` and is also where the cross-rule first appears; see [Forward-compatibility note](#forward-compatibility-note-planned-for-v3).
...
| `pair_matchup_idempotency` | str | `"none"`, `"once_per_league"` | `"once_per_league"` (unchanged from v1) |
| `one_pair_per_player` | bool | `true` only in v2 (any other value is rejected); `false` becomes selectable in v3 | `true` (unchanged from v1) |
| `ranking_subject` | str | `"pair"`, `"player"` | `"pair"` |
...
The defaults reproduce v1 behavior exactly: a `(pair, [matches_won])` league sorts identically to today. This means existing leagues backfilled to v2 (see §[Migration plan](#migration-plan-v1--v2)) produce byte-identical standings to before the migration.
...
2. `pair_matchup_idempotency` validation is unchanged from v1.
3. `one_pair_per_player` must be `true`. Any other value (including `false`) raises `InvalidLeagueRulesError` (mapped to HTTP 422). This applies to both v1 and v2 inputs — v1 payloads with `one_pair_per_player = false` are rejected on read rather than silently upgraded.
...
There is no `(ranking_subject, one_pair_per_player)` cross-rule in v2; rule 3 makes such a rule redundant. v3 introduces the cross-rule when OTPP=false becomes selectable — see [Forward-compatibility note](#forward-compatibility-note-planned-for-v3).
...
| Field | When `subject_kind == "pair"` | When `subject_kind == "player"` |
|---|---|---|
| `subject_kind`, `rank` | required | required |
| `pair_id`, `player1_nickname`, `player2_nickname` | required | absent |
| `player_id`, `nickname` | absent | required |
| `matches_played`, `wins`, `losses`, `games_won`, `games_lost`, `games_diff`, `win_pct` | required | required |

Clients must read `subject_kind` first and branch their renderer on it. Old clients reading only `pair_id` / `player1_nickname` / `player2_nickname` / `wins` / `losses` will silently break for player-subject leagues. The frontend rollout (Slice 2.6) ships in lockstep with the backend rollout (Slices 2.1-2.4).
...
flowchart LR
    ...
    rules --> calc
    calc -->|"subject_kind = pair"| pairRows[Pair rows]
    calc -->|"subject_kind = player"| playerRows[Player rows]
    pairRows --> sort[Sort by tie_breakers tuple desc]
    playerRows --> sort
    sort --> rank[Assign standard-competition rank]
    rank --> resp[Standings response]
```

`StandingsCalculator.compute(matches, pairs, players, rules)` is pure and stateless. It:

1. Builds a list of subject rows (one per pair, or one per player), each carrying every metric in the allowed set, computed from `matches`.
...
No DB calls, no HTTP, no side effects. The use case (`GetStandingsUseCase`) is responsible for loading `matches`, `pairs`, and `players` via repositories before invoking this service.

---

## Worked example: equivalence under OTPP=true

Three pairs, three matches:
...
**Subject = `pair`, `tie_breakers = ["matches_won", "games_diff"]`:**

| Rank | Pair | Wins | Losses | Games for | Games against | Diff |
|---|---|---|---|---|---|---|
| 1 | A (alice, bob) | 2 | 0 | 12 | 5 | +7 |
| 2 | B (charlie, diana) | 1 | 1 | 9 | 10 | -1 |
| 3 | C (eve, frank) | 0 | 2 | 6 | 12 | -6 |

**Subject = `player`, `tie_breakers = ["matches_won", "games_diff"]`, `one_pair_per_player = true`:**
...
Every metric tuple is identical between pairmates, because they always play together when `one_pair_per_player = true`. The two leaderboards convey the *same information*, just in different presentations. This redundancy is intentional in v2: it lets the player-ranking code path be exercised on real data before the multi-pair-per-player feature ships, without requiring synthetic test data only.
...
```sql
UPDATE leagues
SET rules = rules || '{"version": 2, "ranking_subject": "pair", "tie_breakers": ["matches_won"]}'::jsonb
