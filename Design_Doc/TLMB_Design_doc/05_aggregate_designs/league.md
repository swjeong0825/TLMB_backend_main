...
### Value Object: LeagueTimezone
- Fields: value (string, IANA timezone name)
- Why not a primitive: the timezone is league metadata used by application rules (`once_per_day`) and must be validated before persistence or use in date-boundary calculations.
- Validation / normalization: strips surrounding whitespace and validates with Python `zoneinfo.ZoneInfo`; invalid or blank values raise `InvalidLeagueRulesError` and map to HTTP 422. Existing/omitted values default to `America/Los_Angeles`.
- Immutability notes: immutable after construction. There is no API for changing a league timezone after creation in this iteration.
