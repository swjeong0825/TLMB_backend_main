...
- Name: Match
- Purpose: Own a single confirmed doubles match result — the two opposing pair references, the set score, and all match-level structural invariants.
...
- Match involves two distinct pairs: pair1_id and pair2_id must differ (enforced on `create`)
- Set score is structurally valid: pair1_score and pair2_score must both parse as non-negative integers (enforced on `create` and `edit_score`)
- Note: The "no player on both sides" property is not an explicit invariant of this aggregate. It is derivable from "One Pair Per Player Per League" (enforced by League) + the two-distinct-pairs check above. The Match aggregate does not hold or inspect player lists.

---

## Public Behaviors on Root

### `create(pair1_id: UUID, pair2_id: UUID, set_score: SetScore) -> Match`

- Purpose: Record a confirmed doubles match result for the league.
- Inputs: pair1_id, pair2_id (opaque references to Pair entities inside the League aggregate), set_score (a SetScore value object already validated on construction)
- State changes: sets matchId (new UUID), leagueId (set from context at construction — passed as an aggregate-level field, not a behavior parameter), pair1_id, pair2_id, set_score; no explicit recorded_at field on the domain aggregate — match creation time is tracked by the infrastructure-managed created_at column (see Design Decision appendix)
- Invariants checked:
  - pair1_id ≠ pair2_id (raises error if equal)
...
- Notes: Pair references are not changed by this operation. Only the score is updated.
...
- Fields: value (UUID)
- Why not a primitive: semantically distinct from LeagueId and PairId; prevents accidental cross-type assignment
...
- Fields: pair1_score (string), pair2_score (string)
...
- Validation / normalization: on construction, both pair1_score and pair2_score must be non-empty strings that parse as integers ≥ 0; invalid input raises a domain error
...
|---|---|---|
| MatchRecorded | `Match.create` | matchId, leagueId, pair1_id, pair2_id, set_score |
...
- pair1_id, pair2_id: opaque references to Pair entities inside the League aggregate; Match does not hold player lists or nicknames directly

**League rules:** Whether two pairs may play more than once in a league is governed by `LeagueRules` on the League aggregate and enforced in `SubmitMatchResultUseCase` (not in this aggregate). See [16_league_rules_and_match_policies.md](../16_league_rules_and_match_policies.md).

---

- Match stores pair IDs only. Player nicknames are resolved at read time from the current League state. Admin nickname edits will retroactively affect how historical matches are displayed — this is an accepted trade-off in V1.
- Matches are hard-deleted. No soft-delete or archive mechanism is provided in V1.
- One set per match in V1. SetScore holds a single pair1_score / pair2_score pair. If multi-set support is needed in a future version, SetScore would need to become a list of score pairs, or a separate SetResult entity would be introduced.
